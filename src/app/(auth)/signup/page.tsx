...
import CustomPhoneInput from "@/components/others/CustomPhoneInput"; // Adjust the import path as necessary
...
export default function SignUp() {
  ...
  const sendVerification = async (resend = false) => {
    console.log("Phone number at SignUp:", phoneNumber);
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      setToastMessage("LA세탁에 오신걸 환영합니다");
      setShowToast(true);
      setTimeout(() => {
        setShowToast(false);
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-white">
      <Toast message={toastMessage} show={showToast} className="z-50" />
      <div className="bg-gray-50 p-6 rounded-lg shadow-md text-black w-full max-w-md">
        <h1 className="text-1xl font-semibold mb-6">전화번호로 시작해주세요!</h1>
        <div className="mb-4 relative">
          ...
          <button
            ...
            className={`mt-3 w-full py-2 rounded-lg ${
              cooldown ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 text-white hover:bg-blue-700"
            }`}
          >
            {verificationSent ? "승인번호 재요청" : "승인번호 요청"}
          </button>
          {cooldown && <p className="mt-2 text-sm text-gray-400">승인번호 재요청은 조금만 기다려주세요.</p>}
        </div>
        <div className="mb-4">
        ...
        <div className="flex justify-center items-center">
          <button onClick={verifyCode} className="py-2 px-6 rounded-lg bg-green-500 text-white hover:bg-green-600" style={{ wordBreak: "keep-all" }}>
            번호 인증
          </button>
        </div>
