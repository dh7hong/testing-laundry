...
const MainView: React.FC = () => {
  ...
  const navigateToUserGuide = () => {
	...
	const viewCoinLaundryNearby = async () => {
		if (shippingAddress === "배송지를 입력해주세요") {
			...
			if (latitude && longitude) {
				const query = new URLSearchParams({ lat: latitude, lng: longitude });
				router.push(`/viewMap/viewMapSelectedAddress?${query.toString()}`);
			} else {
    ...
    <div className="bg-gray-50">
      <div className="px-[20px] pb-[20px] bg-background-normal-alternative w-full max-w-[430px] mx-auto relative">
        ...
        <div className="flex justify-between items-center">
          <div className="text-heading-2 font-semibold">세탁 신청</div>
          <button
